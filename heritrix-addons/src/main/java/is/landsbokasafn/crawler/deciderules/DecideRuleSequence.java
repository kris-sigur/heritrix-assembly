...
/**
 * Variant on the H3 standard version of this that allows us to tweak the logging a bit.
 *
 */
...
public class DecideRuleSequence extends DecideRule implements BeanNameAware, Lifecycle {
    ...
    public DecideResult innerDecide(CrawlURI uri) {
        ...
        if (fileLogger != null) {
            if (result==DecideResult.REJECT && decisiveRuleNumber!=0 ) {
            	String via = "";
            	if (uri.getVia()!=null) {
            		via = uri.getVia().toCustomString();
            	}
                // LBS: Only interested in the rejections other than the initial REJECT ALL
                fileLogger.info(decisiveRuleNumber + " " + decisiveRule.getClass().getSimpleName() + " " + result + " " 
                		+ uri + " " + via);
            }
        }
